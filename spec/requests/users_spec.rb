  ...
  path '/login' do
    post 'Sign in User' do
      ...
      response '200', 'User logged in successfully' do
        before do
          User.create(full_name: 'Martha K', email: 'marty@luxurycarslending.com', password: 'password')
        end
        let(:user) { { user: { email: 'marty@luxurycarslending.com', password: 'password' } } }
        run_test!
      end
  ...
  path '/logout' do
    delete 'Sign out User' do
      ...
      response '200', 'Logged out successfull' do
        before do
          @user = User.create(full_name: 'Martha M', email: 'martha@luxurycarslending.com', password: 'password')
